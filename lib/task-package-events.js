...
class TaskPackageEvents extends EventEmitter {
    ...
    /** 
     * Emit a start event for a task package
     * @param {string} tp_id - Task package ID
     * @param {object} payload - Event payload
     */
    emitStart(tp_id, payload) {
        const eventName = this.createEventName(`start/${tp_id}`)
        if (this.debug) {
            console.log(`📤 Emitting start event: ${eventName}`)
        }
        this.emit(eventName, payload)
    }
    ...
    /** 
     * Emit an update event for a task package
     * @param {string} tp_id - Task package ID or '*' for any
     * @param {object} payload - Event payload
     */
    emitUpdate(tp_id, payload) {
        const eventName = this.createEventName(`update/${tp_id}`)
        if (this.debug) {
            console.log(`📤 Emitting update event: ${eventName}`)
        }
        this.emit(eventName, payload)
    }
     ...
     * @returns {string} Event name for cleanup
     */
    onStart(tp_id, callback) {
        const eventName = this.createEventName(`start/${tp_id}`)
        if (this.debug) {
            console.log(`📥 Listening for start event: ${eventName}`)
        }
        this.on(eventName, callback)
        return eventName
    }
     ...
     * @param {string} tp_id - Task package ID to listen for or '*' for any
     * @param {function} callback - Event handler function
     * @returns {string} Event name for cleanup
     */
    onUpdate(tp_id, callback) {
        const eventName = this.createEventName(`update/${tp_id}`)
        if (this.debug) {
            console.log(`📥 Listening for update event: ${eventName}`)
        }
        this.on(eventName, callback)
        return eventName
    }
    ...
    removeAllEventListeners(eventName) {
        if (this.debug) {
            console.log(`Removing all listeners for: ${eventName}`)
        }
