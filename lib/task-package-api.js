        ...
        this.router.get('/', async (req, res) => {
            try {
                ...
                if (tp_id) {
                    ...
                    // Build complete form URL
                    if (taskPackage.form_url) {
                        taskPackage.form_url = `${this.config.host_url}:${this.config.host_port}/dashboard/${taskPackage.form_url}`;
                    }
                    ...
                    // Build complete form URLs for all task packages
                    taskPackages.forEach(pkg => {
                        if (pkg.form_url) {
                            pkg.form_url = `${this.config.host_url}:${this.config.host_port}/dashboard/${pkg.form_url}`;
                        }
                    });
    ...
    async validateOIDCToken(token, expectedUser) {
        try {
            ...
            if (this.config.keycloak_url) {
                ...
                userinfoUrl = this.buildUserinfoUrl(baseUrl);
            } else {
                // Fallback: temporarily hardcode for testing
                userinfoUrl = 'http://10.233.0.80:8080/realms/chart-sandbox/protocol/openid-connect/userinfo';
            }
            
            if (!userinfoUrl) {
                console.log('🔓 OIDC provider URL not configured, allowing all requests');
                return { valid: true, user: 'anonymous' }; // Skip validation if no provider URL
            }
