            ...
            try {
                ...
                if (tp_id) {
                    ...
                    const filteredTaskPackages = userTpAllowed.length > 0 
                        ? taskPackages.filter(tp => userTpAllowed.includes(tp.tp_id))
                        : taskPackages;
        ...
        try {
            ...
            if (requiredTpId && userData.tp_allowed) {
                if (!userData.tp_allowed.includes(requiredTpId)) {
                    console.log('❌ User not authorized for TP:', requiredTpId, 'User allowed TPs:', userData.tp_allowed);
                    return { valid: false, message: `Not authorized for task package: ${requiredTpId}` };
                }
                console.log('✅ User authorized for TP:', requiredTpId);
            }
