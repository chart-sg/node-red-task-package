...
/** 
 * Make API call to create task package
 * @param {object} data - The TP creation data
 * @param {object} config - Node configuration
 * @returns {Promise<object>} API response
 */
async function callCreateAPI(data, config) {
    try {
        ...
        if (mod.debug) {
            console.log('🚀 Creating TP via API:', endpoint, requestData)
        }
...
function nodeInstance(config) {
    ...
    this.on('input', async function(msg, send, done) {
        try {
            ...
            this.status({fill: 'yellow', shape: 'dot', text: 'Creating...'})
            ...
            const result = await callCreateAPI(tpData, apiConfig)
            
            // Success status
            this.status({fill: 'green', shape: 'dot', text: `Created: ${result.tpc_id?.substr(0, 8)}...`})
